...
class Clone:
    ...
    async def channels_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for channel_text in guild_from.text_channels:
            if channel_text.category is None:
                print_warning(f"Kanalın {channel_text.name} herhangi bir kategorisi Yok, atlanıyor")
                continue
            try:
                ...
                if category is None:
                    print_warning(f"Kanalın {channel_text.name} için uygun kategori bulunamadı, atlanıyor")
                    continue
        ...
        for channel_voice in guild_from.voice_channels:
            if channel_voice.category is None:
                print_warning(f"Kanalın {channel_voice.name} herhangi bir kategorisi Yok, atlanıyor")
                continue
            try:
                for category in guild_to.categories:
                ...
                if category is None:
                    print_warning(f"Kanalın {channel_voice.name} için uygun kategori bulunamadı, atlanıyor")
                    continue
    ...
    async def emojis_delete(guild_to: discord.Guild):
        for emoji in guild_to.emojis:
            try:
                ...
                print_error(f"Emoji Silinemiyor: {emoji.name}")
    ...
    async def guild_edit(guild_to: discord.Guild, guild_from: discord.Guild):
        try:
            try:
                ...
                print_error(f"Simge görüntüsü okunamıyor: {guild_from.name}")
                icon_image = None
